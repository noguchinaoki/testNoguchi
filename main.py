...
students.student_id = scores.student_id で結合できます。
...
# --- 入力と応答処理 ---
if prompt := st.chat_input("質問をどうぞ（例：田中亮介の点数を100に変更して）"):
    st.session_state.chat_history.append({"role": "user", "content": prompt})
    with st.chat_message("user"):
        st.markdown(prompt)

    # --- SQL生成対象かどうかを判定 ---
    if any(keyword in prompt for keyword in ["田中亮介", "点数", "成績", "データ", "変更", "修正", "削除", "登録", "追加"]):
